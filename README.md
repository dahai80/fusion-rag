...
| `FUSION_RAG_REQUIRE_IDENTITY` | (empty) | Set `1` to enable authoritative tenant resolution via fusion-identity (#68): `/kb/*` requests require a valid `Authorization: Bearer <JWT>` verified by the identity service, and the tenant is taken from the JWT `tid` claim. `X-Fusion-Tenant` is demoted to defense-in-depth (must match the JWT `tid` if present). Default off. |
...
| `FUSION_IDENTITY_SERVICE_TOKEN` | (empty) | Service token sent as `Authorization: Bearer` to fusion-identity `/api/v1/auth/verify`. Required when `FUSION_RAG_REQUIRE_IDENTITY=1` (fail-closed if unset). |
...
**Authoritative tenant resolution (#68).** The #61 path trusts the `X-Fusion-Tenant` header, which is forgeable by any direct-port caller. When `FUSION_RAG_REQUIRE_IDENTITY=1`, fusion-rag resolves the tenant **authoritatively** from a fusion-identity JWT instead: every `/kb/*` request must carry `Authorization: Bearer <JWT>`, which fusion-rag verifies via fusion-identity's `POST /api/v1/auth/verify` (service-token-gated; revocation and tenant-status enforced by the identity service). The JWT `tid` claim becomes the authoritative tenant and feeds the same `_request_tenant` contextvar, so KB scoping (#61) and per-tenant collection isolation (#66) keep working unchanged. `X-Fusion-Tenant` is demoted to defense-in-depth: if present it must equal the JWT `tid` (mismatch → 401, a blocked forgery attempt). Requests with no/invalid/revoked JWT are 401; an unreachable identity service is fail-closed (deny, not fallback to the header). fusion-identity is an HTTP service dependency (port 11470), not a Python import — fusion-rag talks to it over plain HTTP. Default off; single-tenant local-first dev is unaffected.
...
- **Authoritative tenant resolution via fusion-identity (#68)** — retires the forgeable `X-Fusion-Tenant` blind-trust from #61. When `FUSION_RAG_REQUIRE_IDENTITY=1`, `/kb/*` requests require `Authorization: Bearer <JWT>`; fusion-rag verifies the JWT via fusion-identity's `POST /api/v1/auth/verify` (service-token-gated, revocation + tenant-status enforced by the identity service) and takes the authoritative tenant from the JWT `tid` claim. The resolved tenant feeds the same `_request_tenant` contextvar, so KB scoping (#61) and per-tenant collection isolation (#66) keep working unchanged. `X-Fusion-Tenant` is demoted to defense-in-depth (must match the JWT `tid` if present, else 401). No/invalid/revoked JWT → 401; unreachable identity → fail-closed deny. fusion-identity is an HTTP service dependency (port 11470), not a Python import — fusion-rag talks to it over plain HTTP. Default off; single-tenant local-first dev unaffected. 12 tests (forged header 401, invalid JWT 401, valid JWT scoped, cross-tenant 404, header/JWT mismatch 401, revoked 401, exempt paths, integration-off regression guard, client unit).
